...
extension ParkingPlaceItemDTO {
    func convert() -> ParkingPlace {
        let number = Int(self.parkingPlaceNumber.components(separatedBy: "-").joined()) ?? 0
        let operatingDays = self.operatingDays.components(separatedBy: "+")
        let chargeType = self.parkingchargeInformation == "무료" ? 0 : 1
        let baseTime = Int(self.basicTime) ?? 0
        let baseAmount = Int(self.basicCharge) ?? 0
        let additionalTime = Int(self.additionalUnitTime) ?? 0
        let additionalAmount = Int(self.additionalUnitCharge) ?? 0

        return ParkingPlace(
            number: number,
            name: self.parkingPlaceName,
            roadAddress: self.roadAddress,
            jibunAddress: self.jibunAddress,
            weekdayOperating: operatingDays.contains("평일"),
            saturdayOperating: operatingDays.contains("토요일"),
            holidayOperating: operatingDays.contains("공휴일"),
            weekdayOpenTime: self.weekdayOperationOpenTime,
            weekdayCloseTime: self.weekdayOperationCloseTime,
            saturdayOpenTime: self.saturdayOperationOpenTime,
            saturdayCloseTime: self.saturdayOperationCloseTime,
            holidayOpenTime: self.holidayOperationOpenTime,
            holidayCloseTime: self.holidayOperationCloseTime,
            chargeType: chargeType,
            baseChargeTime: baseTime,
            baseChargeAmount: baseAmount,
            additionalChargeTime: additionalTime,
            additionalChargeAmount: additionalAmount,
